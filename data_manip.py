...
class preprocessor():
    ...
    def collect_paths(self, input_path):
        ...
        print("----------------")
        print("\n".join(label_paths))
        return coord_paths, label_paths
    
    def normalize(self,data):
        return 2 * (data - np.min(data)) / (np.max(data) - np.min(data)) - 1

    def generate_matrices(self, sample_size = 2048, number_of_samples_per_label = 32, set_name = 'train',feature = 5):
        ...
        for scene in range(len(self.coord_paths)): #len(self.coord_paths)
            ...
            if len(label_point_cloud) > sample_size:
                ...
                print(f'{self.coord_paths[scene]} has less than {sample_size} points for label {feature}... padding')
                for sample_number in range(number_of_samples_per_label):
        ...
        np.save(f"{name_table[feature]}_{set_name}.npy", normalized_stacked_point_cloud_objects, allow_pickle=False)
                
            


mypreprocessor = preprocessor("/share/lsmsmart/regarry/Terrain3DPointCloudGeneration/data/test")
mypreprocessor.generate_matrices(2048, 64, 'train', feature = 5)
